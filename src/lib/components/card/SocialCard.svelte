...
<script lang="ts">
  ...
  function b64Encode(str: string) {
    if (!browser) return str.replace(/./g, "•");
  ...
  const displayHandle = $derived(
    revealed && isEmail ? b64Decode(encodedHandle) : encodedHandle,
  );
...
{#snippet cardIcon()}
  ...
  <img
    src={icon}
    alt={platform}
    class="h-8 w-8 p-1.5 rounded-md {iconDark ? 'hidden dark:block' : ''}"
  ...
  {#if iconDark}
    <img
      src={iconDark}
      alt={platform}
      class="dark:hidden h-8 w-8 p-1.5 rounded-md"
      style="background-color: var(--bg-icon)"
    />
  {/if}
{/snippet}
...
{#snippet cardHandle(desktopHover: boolean)}
  <div>
    ...
    {#if isEmail}
      <p class="inline-flex" aria-label={revealed ? handle : "Email address"}>
        {#each displayHandle.split("") as char, i}
          <span
            class="shatter-char relative inline-block"
            class:revealed
            style="--i: {i}; --scramble: '{scrambledChars[i] ?? '•'}'"
            >{char}</span
          >
        {/each}
      </p>
